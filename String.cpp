...
TString* TString::Split(char c)
{
	...
	for (int i = 0; i < length; i++)
	{
		if (string[i] == c)
		{
			strings[kk].length = i - start + 1;
			delete[] strings[kk].string;
			strings[kk].string = new char[strings[kk].length];
			for (int j = 0; j < strings[kk].length - 1; j++)
				strings[kk].string[j] = string[j + start];
			strings[kk].string[strings[kk].length] = '\0';
			kk++;
			start = i + 1;
		}
	}
	strings[kk].length = length - start;
	delete[] strings[kk].string;
	strings[kk].string = new char[strings[kk].length];
	for (int k = 0; k < strings[kk].length; k++)
		strings[kk].string[k] = string[k + start];
	kk++;
	return strings;
}
